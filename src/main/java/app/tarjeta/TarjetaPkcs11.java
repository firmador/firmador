...
public class TarjetaPkcs11 {
	...
	public void get_id_certificados(){
	  ...
	  public PKCS11 getPkcs11Manager(String rutaLibreriaSmartcard) throws IOException, PKCS11Exception
	  {
	    if(pkcs11==null)
	  ...
	  @SuppressWarnings("restriction")
	  public List<X509Certificate> getCertificates(long numeroDeSlot) throws IOException, PKCS11Exception, CertificateException
	  {
	    List<X509Certificate> certificates = new ArrayList();
	    PKCS11 pkcs11 = getPkcs11Manager(Utils.getPKCS11Lib());	    
	    long sessionHandle = pkcs11.C_OpenSession(numeroDeSlot, 4L, null, null);
	    ...
	    for (long i : objectHandles)
	    {
	      ...
	      //type = 258L;
	      byte[] p2Value = get_pkcs11_certificate_attr(sessionHandle, i, pkcs11);
	      
	      if (p2Value != null) {
	        attrsP[0] = attrPriv;
	        attrsP[1] = attr;
	        pkcs11.C_FindObjectsInit(sessionHandle, attrsP);
	        certificates.add(extrae_certs_pkcs11(sessionHandle, i, pkcs11));
	        pkcs11.C_FindObjectsFinal(sessionHandle);
	      }
	    }
	  ...
	  @SuppressWarnings("restriction")
	  private byte[] get_pkcs11_certificate_attr(long session, long oHandle, PKCS11 pkcs11)
			    throws PKCS11Exception{
			    byte[] bytes = null;
			    CK_ATTRIBUTE[] atributos = { new CK_ATTRIBUTE(258L) };
	...
	@SuppressWarnings("restriction")
	private X509Certificate extrae_certs_pkcs11(long session, long oHandle, PKCS11 pkcs11)
			    throws PKCS11Exception, CertificateException
			  {
			    ...
			    X509Certificate certificado = (X509Certificate)certificateFactory.generateCertificate(new ByteArrayInputStream(bytes));
