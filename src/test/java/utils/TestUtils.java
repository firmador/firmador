...
public class TestUtils {
    ...
    public static void createDirectoryWithNoAccess(String path){
        try {
            ...
            if (System.getProperty("os.name").toLowerCase().contains("windows")) {
                final Process p = Runtime.getRuntime().exec("icacls " + path + " /deny \"" + System.getProperty("user.name") + ":(W,WD,AD)\" /t /inheritance:r");
                p.waitFor();  // wait for it to end before continue with the next line

                System.out.println("----------");
                System.out.println("icacls " + path + " /deny \"" + System.getProperty("user.name") + ":(W,WD,AD)\" /t /inheritance:r");
                BufferedReader is =
                    new BufferedReader(new InputStreamReader(p.getInputStream(  )));
                String s;
                while ((s = is.readLine()) != null) {
